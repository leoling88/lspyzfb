...
<script>
  ...
  export default {
    ...
    created() {
      this.$bus.$off('showDialogPopup');
      this.$bus.$on('showDialogPopup', (data) => {
          this.showPopup=true;
          this.clearNum();//清空原来密码
      });
    },
    methods: {
      findPayPwd(){//找回密码链接
...
<style lang="less" scoped>
  ...
  .num_name_box span{ line-height: 10px; background: url("../../assets/images/me/zf_ico.png") left center no-repeat; background-size: 0.30rem 0.33rem;padding: 5px 0 5px 0.35rem; }
  ...
  .form_edit> div:last-child {  background-image:url("../../assets/images/me/del_ico.jpg");  background-repeat: no-repeat;  background-position: center center;  background-size: 0.56rem 0.38rem;  }
</style>
