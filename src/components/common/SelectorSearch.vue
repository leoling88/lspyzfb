<template>
  <div class="selector_search_box">
    ...
    <template>
      <x-input v-if="value == -1111111111" v-model.trim="entryValue" :title="describe" :placeholder="`请输入${describe}`" :show-clear="true" :required="true" placeholder-align="right" text-align="right" @on-change="entryValChange" class="entry_input">
        <span v-if="false" slot="right" @click="showPopup">　选择▲</span>
      </x-input>

      <cell v-else class="custom_cell" :title="describe" :value="itemName"  :is-link="isLink"  @click.native="showPopup" :disabled="disabled" :is-loading="this.value.length > 1 && this.options.length <= 0">
    ...
    <div :class="['popup_cont',show && !disabled ? 'active' : '', showLoadMore ? 'loadMore' : '']" @scroll="scrollFn" ref="searchList">
      ...
      <template>
        <!--关键字搜索-->
        <div v-show="hasSearch" :class="['search_box',isFiexd ? 'isFiexd' : '']">
        ...
        <ul :class="['list_cont', hasSearch ? 'hasSearch' : '']" ref="searchListCont" @click="closePopup">
          <li v-if="hasEntry" @click.self="checkItem(-1, '暂无,请手动输入')" key="-1" style="color:#FF0000;">暂无,请手动输入<icon type="success-no-circle"></icon></li>
...
//        this.$emit('change',val, this.itemName, this.unitAdress, this.unitNumber, this.unitPhone);
    ...
    methods: {
      entryValChange () {
        this.$emit('entryValChange', this.entryValue)
      },
...
//        this.$refs.searchList.scrollTop = 0
      ...
      checkItem (key, val, dwdz, qybh, lxdh) {
        if (this.itemName != val || (qybh && this.unitNumber != qybh)) {
          this.itemName = val;
...
<style lang="less" scoped>
  ...
  .vux-x-input.search_input{
    width: 100%;background:#FAFAFA;padding:0px 0;margin: 0 0px;border-bottom:1px solid #eee;
    .weui-cell__bd{flex: 6;}
  }
  ...
  .list_cont.hasSearch{padding-top:1rem;}
